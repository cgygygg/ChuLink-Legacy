  ...
  async function draftStory(input, hooks = {}) {
    ...
    const body = {
      ...
      messages: [
        {
          role: 'system',
          content: '你是楚韵链迹的文化内容编辑。只能依据给定且已审核的资料来源，写一张直接放在用户投稿下方的短故事卡：标题使用具体的人、物、地点或动作，8 至 16 个汉字；导语 30 至 50 个汉字；正文只写 1 至 2 个短章节，每章 70 至 140 个汉字。第一句直接写现场细节，不做宏大抒情。禁止“穿越千年”“历史长河”“文化瑰宝”“承载深厚底蕴”“见证时代变迁”等套话，不重复同一个结论。每章必须引用至少一个 sourceLinkIds，且只能使用输入中存在的 linkId；章节中的年代、数字、人物和地点必须在所引来源中直接出现。supplements 只说明投稿新增了某类已审核资料，不能猜测图片、音频或视频中的具体内容。不得补写输入中没有的历史结论；证据有限时使用“记录显示”“投稿者观察到”等克制表述。严格按照 JSON Schema 输出。'
        },
        {
          role: 'user',
          content: JSON.stringify({
