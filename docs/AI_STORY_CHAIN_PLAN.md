...
- 已发布故事末尾直接提供“纠正或补充”，不新增顶层入口；
- 用户可区分“内容可能有误、建议补充来源、表述需要调整”，并定位整篇或具体章节；
- 反馈由服务端绑定资源、故事 ID、故事版本和章节，客户端不能伪造章节标题；
- 故事刚被新版本替换时，旧版本反馈仍可保留，避免用户填写内容丢失；
- 管理后台展示故事版本上下文，可回复、关闭或标记“纳入故事修订”；
- 处理结果进入用户通知和管理日志，个人中心可查看状态与回复；
- 反馈不会自动修改原故事，也不会绕过现有故事草稿审核与发布流程。

下一阶段先继续完善统一资源详情中的成熟长文与路线步骤，再做基于明确规则的游览偏好排序。OCR、语音转写和生成式讲解仍需单独授权与成本确认。

## 第七阶段实现记录（2026-08-17）

已完成“故事质量门槛 v2”：

- 生成前使用确定性规则检查已确认来源、现场描述、关系说明、材料完整度和证据类型，资料不足时不调用模型；
- 故事提示词升级为 `sourced-story-contract-v2`，要求具体开头、克制表达和逐章来源约束；
- 生成后检查章节来源、敏感信息、空泛套话、重复句，以及来源中没有直接出现的年代和数字；
- 来源、结构或隐私存在硬性错误时不保存草稿；普通问题以“校读签”交给管理员修改；
- 发布时根据管理员修改后的正文重新检查，低于建议线需要填写人工判断说明，且说明与质量结果一同写入版本日志；
- 用户端不展示分数和内部错误，只显示“已人工校读”或一句明确的补充建议，不增加新的顶层入口。

质量规则当前以人工可解释的确定性检查为主，不使用同一个模型给自己的故事打分。后续应先观察真实草稿分布，再调整阈值和词表。
